...
const AddLoanPaymentScreen = ({ navigation, route }) => {
  const { loan } = route.params;
  const { recordPartialLoanPayment } = useFinance();
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (paymentAmount > remainingBalance) {
        Alert.alert('Error', `Payment amount cannot exceed remaining balance of ${formatCurrency(remainingBalance)}`);
        return;
      }
      ...
      const success = await recordPartialLoanPayment(loan.id, paymentData);
...
export default AddLoanPaymentScreen;
